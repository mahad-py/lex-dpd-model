...
ALL_FEATURES = CAT_COLS + A_COLS + B_COLS + C_COLS
...
def train_model(df_hash):
    ...
    imp   = pd.Series(clf.feature_importances_, index=ALL_FEATURES).sort_values(ascending=False)
    report = classification_report(y_test, pred, output_dict=True)
    return clf, acc, f1, imp, report
...
if uploaded:
    ...
    with st.spinner("Training DPD behavioural scorecard model..."):
        clf, acc, f1, importance, report = train_model(str(len(df)))
    st.sidebar.metric("Model accuracy",  f"{acc*100:.2f}%")
    st.sidebar.metric("Macro F1 score",  f"{f1*100:.2f}%")
    st.sidebar.caption("Trained on approved applicants only — predicts post-approval DPD behaviour")
else:
    st.info("👈 Upload the GCC_Approved_10K_ABC dataset in the sidebar to train the model.")
    st.stop()
...
# ══════════════════════════════════════════════════════════════
with tab1:
    st.subheader("DPD Outcome Distribution in Training Data")
    ...
    with col1:
        dpd_counts = df['dpd_outcome'].value_counts().reset_index()
        dpd_counts.columns = ['DPD Outcome','Count']
        fig1 = px.bar(dpd_counts, x='DPD Outcome', y='Count',
                      title="Applicant Count by DPD Outcome",
                      color='DPD Outcome',
                      color_discrete_map={
                          'current':'#2f9e6e','dpd_30':'#c9a35a',
                          'dpd_60':'#d98c2b','dpd_90plus':'#c2452f','written_off':'#0f2138'})
        st.plotly_chart(fig1, use_container_width=True)

    with col2:
        dpd_by_band = df.groupby('risk_band')['dpd_outcome'].apply(
            lambda x: (x.isin(['dpd_90plus','written_off'])).mean()*100).reset_index()
        dpd_by_band.columns = ['Risk Band','NPL %']
        fig2 = px.bar(dpd_by_band, x='Risk Band', y='NPL %',
                      title="NPL Rate (90+/Writeoff) by Risk Band",
    ...
    for cls in DPD_CLASSES:
        if cls in report:
            r = report[cls]
            perf_rows.append({
                'DPD Class': f"{DPD_ICONS.get(cls,'')} {cls}",
...
with tab2:
    ...
    st.caption("This applicant has already been approved by the rule engine. This model predicts their post-approval repayment behaviour.")
    ...
    if submitted:
        ...
        requested_amount  = approved_amount * 1.2  # assumed they asked for more
        ...
        row = pd.DataFrame([{
            ...
            'other_monthly_income_sar':other_monthly_income_sar,
            'total_monthly_income_sar':total_income,
            'existing_monthly_obligations_sar':existing_monthly_obligations_sar,
            'nafath_verified':nafath_verified,'yakeen_match':yakeen_match,
            ...
            'avg_eom_balance_sar':avg_eom_balance_sar,
            ...
            'digital_txn_ratio':digital_txn_ratio,
            'avg_monthly_inflow_sar':avg_monthly_inflow_sar,
            'avg_monthly_outflow_sar':avg_monthly_outflow_sar,
            'prev_loans_count':prev_loans_count,
            'prev_defaults_count':prev_defaults_count,
            'max_dpd_last_loan':max_dpd_last_loan,
            'ever_restructured':ever_restructured,
            'ever_written_off':ever_written_off,
            'simah_enquiries_3m':simah_enquiries_3m,
        ...
        npl_prob    = sum(proba[classes.index(c)] for c in ['dpd_90plus','written_off'] if c in classes)*100
        ...
        with r1:
            icon = DPD_ICONS.get(pred,'⚪')
            st.markdown(f"### {icon} DPD Prediction")
        ...
        b_payments   = round(max(0,100-missed_payments_6m*25))
        b_score      = round(b_salary*0.25+b_balance*0.20+b_cash*0.15+b_util*0.25+b_payments*0.15)

        # C scores
        c_history    = round(max(0,100-prev_defaults_count*40))
        c_dpd        = round(max(0,100-(max_dpd_last_loan/180)*100))
        c_writeoff   = 0 if ever_written_off else 100
        c_restructure= 60 if ever_restructured else 100
        c_enquiries  = round(max(0,100-simah_enquiries_3m*15))
        c_score      = round(c_history*0.30+c_dpd*0.25+c_writeoff*0.25+c_restructure*0.10+c_enquiries*0.10)
        ...
        scorecard = [
            ...
            {"Layer":"B — Behaviour",  "Component":"Payment Consistency (6m)",    "Score":f"{b_payments}/100", "Status":badge(b_payments)},
            {"Layer":"B — Behaviour",  "Component":"B Layer Score",               "Score":f"{b_score}/100",    "Status":badge(b_score)},
            {"Layer":"C — Collection", "Component":"Previous Default History",    "Score":f"{c_history}/100",  "Status":badge(c_history)},
            {"Layer":"C — Collection", "Component":"Max Days Past Due",           "Score":f"{c_dpd}/100",      "Status":badge(c_dpd)},
            {"Layer":"C — Collection", "Component":"Write-off History",           "Score":f"{c_writeoff}/100", "Status":"✅ Clean" if not ever_written_off else "🔴 Written Off"},
            {"Layer":"C — Collection", "Component":"Loan Restructuring",          "Score":f"{c_restructure}/100","Status":"✅ Clean" if not ever_restructured else "⚠️ Restructured"},
            {"Layer":"C — Collection", "Component":"SIMAH Enquiries (3m)",        "Score":f"{c_enquiries}/100","Status":badge(c_enquiries)},
...
with tab3:
    ...
    st.caption("Upload a file of approved applicants with A+B+C features. Get DPD prediction for each.")
